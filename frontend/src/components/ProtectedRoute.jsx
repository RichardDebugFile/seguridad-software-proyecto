...
/**
 * Componente para proteger rutas - BALANCEADO entre seguridad y usabilidad
...
function ProtectedRoute({ children, requireAdmin = false }) {
  ...
  useEffect(() => {
    ...
    const handlePopState = async () => {
      console.log('🔙 Navegación retroceder detectada');
      preventBack(); // Re-pushear estado

      // CRÍTICO: Validar con backend para verificar si la sesión sigue activa
      const isValid = await validateWithBackend();
      if (!isValid) {
        console.log('⚠️ Sesión inválida al retroceder, redirigiendo...');
        redirectToLogin();
      }
    };
    ...
    const handleVisibilityChange = async () => {
      if (document.visibilityState === 'visible') {
        console.log('👁️ Página visible, validando sesión con backend...');
        const isValid = await validateWithBackend();
        if (!isValid) {
          console.log('⚠️ Sesión inválida al volver, redirigiendo...');
          redirectToLogin();
        }
      }
    };
    ...
    const handleStorageChange = (e) => {
      if (e.key === 'logout-event' && e.newValue) {
        console.log('🚪 Logout detectado desde otro portal');
        redirectToLogin();
      }
    };
    ...
    const intervalId = setInterval(() => {
      if (!validateToken()) {
        console.log('⚠️ Token inválido detectado en interval');
        redirectToLogin();
      }
    }, 2000);
